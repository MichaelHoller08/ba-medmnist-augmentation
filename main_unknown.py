...
# Ziel-Daten einmalig für N=50 fixieren
set_seed(CURRENT_SEED)
small_train_ds = get_balanced_subset(train_dataset, NUM_SAMPLES_PER_CLASS)
print(f"Ziel-Trainingsdaten geladen: {len(small_train_ds)} Bilder (N={NUM_SAMPLES_PER_CLASS} pro Klasse).\n")


# ==========================================
# 2. DIE SCHLEIFE FÜR DIE ABLATIONSSTUDIE
# ==========================================
for num_unknown in UNKNOWN_AMOUNTS_TO_TEST:
    print("="*60)
    print(f"STARTE TEST MIT UNKNOWN-MENGE: {num_unknown}")
    print("="*60)
    
    set_seed(CURRENT_SEED)
    
    # Unbekannt-Daten für diesen Durchlauf zusammenstellen
    if num_unknown > 0:
        blood_indices = np.random.choice(len(blood_train_ds), num_unknown, replace=False)
        small_blood_ds = Subset(blood_train_ds, blood_indices)
        unknown_ds = LabelWrapper(small_blood_ds, 9)
    else:
        # Falls 0 Unknowns getestet werden, machen wir ein leeres Dummy-Dataset oder ignorieren es
        unknown_ds = None

    # ------------------------------------------
    # BASELINE EXPERIMENT (Erzeugt bei num_unknown=0 Sinn, 
    # läuft aber bei jedem Durchlauf mit als Referenz)
    # ------------------------------------------
    ...
    # ------------------------------------------
    print(f"\n--- STRATEGIE (mit {num_unknown} Unknowns): Optuna Tuning ---")
    def objective_strategy(trial):
        ...
        for train_idx, val_idx in skf.split(np.zeros(len(target_labels)), target_labels):
            ...
            # Hier greift die Variable num_unknown!
            if num_unknown > 0:
                l_tr_c = ConcatDataset([l_tr_t, unknown_ds])
            else:
                l_tr_c = l_tr_t # Fallback, falls num_unknown = 0
            ...
            # Phase 1
            model = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
            model.fc = nn.Linear(model.fc.in_features, 10 if num_unknown > 0 else 9)
            model = model.to(DEVICE)
            optimizer = optim.Adam(model.parameters(), lr=lr)
            criterion = nn.CrossEntropyLoss()
    ...
    print(f"--- STRATEGIE (mit {num_unknown} Unknowns): Finales Training ---")
    if num_unknown > 0:
        comb_train_final = ConcatDataset([small_train_ds, unknown_ds])
    else:
        comb_train_final = small_train_ds
    ...
    strat_model.fc = nn.Linear(strat_model.fc.in_features, 10 if num_unknown > 0 else 9)
    ...
    experiment_data = {
        ...
        "Anzahl Unknown Samples": num_unknown,  # <--- Hier landet jetzt der jeweilige Wert!
    ...
    print(f"\n--- TEST MIT {num_unknown} UNKNOWNS ERFOLGREICH ABGESCHLOSSEN UND GESPEICHERT ---\n")

print("\n==========================================")
print("ABLATIONSSTUDIE FÜR UNKNOWN-MENGEN ABGESCHLOSSEN!")
print("==========================================")